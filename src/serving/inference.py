...
# Try to load model - with fallback for local development
model = None
try:
    ...
    try:
        import glob
        # Search for latest model in local mlruns
        ...
        if local_model_paths:
            ...
            # Try searching from parent directory (when running from scripts/)
            parent_model_paths = glob.glob("../mlruns/*/*/artifacts/model")
            if parent_model_paths:
                latest_model = max(parent_model_paths, key=os.path.getmtime)
                model = mlflow.pyfunc.load_model(latest_model)
                MODEL_DIR = latest_model
                print(f"✅ Fallback: Loaded model from {latest_model}")
            else:
                raise Exception("No model found in ./mlruns or ../mlruns")
    except Exception as fallback_error:
        print(f"❌ All model loading attempts failed!")
        print(f"   Primary error: {e}")
        print(f"   Fallback error: {fallback_error}")
        print(f"   Please train a model first using: python scripts/run_pipeline.py --input data/raw/Telco-Customer-Churn.csv --target Churn")
        model = None  # Set to None, will be checked before prediction
...
try:
    if MODEL_DIR and os.path.exists(MODEL_DIR):
        ...
        if not os.path.exists(feature_file):
            # Try parent artifacts directory
        ...
        if os.path.exists(feature_file):
            ...
            print(f"⚠️  Feature columns file not found, using defaults")
except Exception as e:
    print(f"⚠️  Failed to load feature columns: {e}, using defaults")

# Fallback to default feature columns if not loaded
if FEATURE_COLS is None:
    print("⚠️  Using default feature columns (may not match trained model exactly)")
    FEATURE_COLS = [
