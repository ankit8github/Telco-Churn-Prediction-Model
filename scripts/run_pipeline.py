...
def main(args):
    ...
    # FIX for Windows: Convert to proper file URI format
    if args.mlflow_uri:
        mlruns_path = args.mlflow_uri
    else:
        mlruns_path = os.path.join(project_root, "mlruns")
        # Convert Windows backslashes to forward slashes for URI
        mlruns_path = mlruns_path.replace("\\", "/")
        # Ensure proper file URI format for Windows: file:///C:/path/to/mlruns
        mlruns_path = f"file:///{mlruns_path}"
    ...
    with mlflow.start_run():
        ...
        # Check if data is already processed (has encoded columns)
        is_processed = 'MultipleLines_Yes' in df.columns or all(
            df[col].dtype in ['int64', 'float64', 'bool'] 
            for col in df.columns if col != 'Churn'
        )
        
        if is_processed:
            print("⚠️  Data appears to be pre-processed. Skipping raw data validation.")
            print("✅ Using processed data directly.")
            is_valid = True
            failed = []
        else:
        ...
        # === STAGE 2 & 3: Data Preprocessing & Feature Engineering ===
        if is_processed:
            print("⏭️  Skipping preprocessing and feature engineering (data already processed)")
            df_enc = df
            target = args.target
            ...
            processed_path = os.path.join(project_root, "data", "processed", "churn_processed_data.csv")
...
if __name__ == "__main__":
    ...
    p.add_argument("--input", type=str, required=True,
                   help="path to CSV (e.g., C://Users//debru//Churn Prediction Model//.gradio//flagged//Churn dataset.csv)")
